...
export const InputForm: React.FC<InputFormProps> = ({ 
    ...
    isAnalyzing, analysisError, analysisSuccess, onAnalyzePdf
}) => {
                ...
                <Card title="個別部材の設定">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
                        ...
                        <div className="space-y-3 p-4 bg-slate-50 rounded-lg">
                            ...
                            <div className="pt-2 text-sm font-medium text-slate-600">支柱構成の合計高さ: H’ {totalPillarHeight} mm</div>
                            {validation.pillarHeightStatus === 'ok' ? (
                                <Alert type="success" message="支柱構成と足場高さが一致" />
                            ) : (
                                <Alert type="error" message="支柱構成と足場高さが不一致" />
                            )}
                        </div>
                        {/* Jack Base */}
                        <div className="space-y-3 p-4 bg-slate-50 rounded-lg">
